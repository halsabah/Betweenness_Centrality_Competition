...
class Plotter:
    ...
    def get_images(self, game):
        ...
        for p in game.players:
            pl = game.players[p]
            images[pl.node_id] = pl.picture
        return images

    def get_labels(self, game):
        """
        Compute the images of the players according to their entity type to be able to easily differentiate them
        :param game: Game, played game
        :return: string of color initials
        """
        labels = {}
        # replace by risk to be infected (proba ** nb of infected neighbors)

        for i in range(len(game.graph.nodes())):
            labels[i] = str(i)
    ...
    def get_graph_labels_sizes(self, game, round_number, node_list=None):
        ...
        return current_graph, labels, sizes, img
    ...
    #     plt.axis('off')
    #
    #     draw_networkx(current_graph, positions, labels=labels,
    #                      node_color=colors, node_size=sizes, alpha=self.node_transparency, fig=fig, images=images)
    #
    ...
    def plot_state(self, game, node_list=None, block=True):
        ...
        :return: void
        """
        positions = self.get_positions(len(game.graph.nodes()))
        colors = self.get_colors(game)
        img = self.get_images(game)
        labels = self.get_labels(game)
        ...
        # nx.draw_networkx(game.graph, pos=positions, node_color=colors)

        _display_graph(game.graph, positions, colors=colors, fig=fig, img=img, labels=labels)

        # _display_graph(game.graph, positions, labels, colors, sizes, alpha, leader_board=leader_board_str,
    ...
    def plot_game(self, game, interactive=False, time_step=0.05, node_list=None, leader_board=False):
            ...
            if interactive:
                ...
                def key_event(e):
                    ...
                    img = graphs[curr_pos][3]
                    leader_board_str = ''
                    if leader_board:
                        leader_board_str = _get_leader_board(game, curr_pos, self.leader_board_size,
                                                             self.significant_digits)

                    _display_graph(graph, positions, labels, colors, sizes, alpha, leader_board=leader_board_str,
                                   display_labels=self.labels_interactive_graph, game=game, fig=fig, img=img)

                    mng = plt.get_current_fig_manager()
                    mng.resize(*mng.window.maxsize())

                    # _display_graph(graph, positions, colors=colors, fig=fig, img=img)
                ...
                for round_number in range(len(game.history)):
                    ...
                    _display_graph(graph, positions, labels, colors, sizes, alpha, leader_board=leader_board_str,
                                   display_labels=True, game=game, fig=fig, img=img)
...
def _display_graph(graph, positions=None, labels=None, colors="g", sizes=300, alpha=1, leader_board=None,
                   display_labels=False, **kwargs):
    ...
    draw_networkx(graph, positions, labels=labels, node_color=colors,
...
def draw_networkx_nodes(G, pos,
    ...
    try:
        ...
        print("KeyError exception importing kwarg in draw_networkx_nodes: " + str(e))
    ...
    for node in nodelist:
        # 10 nodes
        # imscatter(pos[node][0], pos[node][1], img[node], label[node], ax=ax, zoom=0.07)
        # 20 nodes
        imscatter(pos[node][0], pos[node][1], img[node], label[node], ax=ax, zoom=0.05)
        ax.plot(pos[node][0], pos[node][1])
    # plt.show()


    # for coord in xy:
    #     imscatter(coord[0], coord[1], 'centrality/default.jpg', zoom=(node_size/300-1)/10, ax=ax)
...
def imscatter(x, y, image, label, ax=None, zoom=1):
    if ax is None:
        ax = plt.gca()
    try:
    ...
    labl = TextArea(label, minimumdescent=False)
